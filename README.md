...
1. Clone the repository and add the `scripts/k8s-helpers` directory to your `PATH`:

    ```bash
    git clone https://github.com/<your-username>/5G-Lab.git
    echo 'export PATH="/path/to/5G-Lab/scripts/k8s-helpers:$PATH"' >> ~/.bashrc
    source ~/.bashrc
    cd /path/to/5G-Lab/
...
```bash
cd /path/to/5G-Lab/testbed_automator/
./install_k8s.sh
```
...
```bash
cd path/to/5G-Lab/manifests/prometheus/
...
```bash
cd path/to/5G-Lab/manifests/open5gs/
...
```bash
cd path/to/5G-Lab/manifests/open5gs/mongo-tools/
...
1. Deploy Open5GS:
    ...
    ```bash
    cd path/to/5G-Lab/manifests/open5gs/open5gs/
...
2. Deploy srsRAN gNB:
    ...
    ```bash
    cd path/to/5G-Lab/manifests/srsRAN/srsran-gnb/
    kubectl apply -k . -n open5gs
...
1. Deploy UEs:
    ...
    ```bash
    cd path/to/5G-Lab/manifests/ues/srsue/
    kubectl apply -k . -n open5gs
    ```
...
```bash
cd path/to/5G-Lab/manifests/iperf/
kubectl apply -k . -n open5gs
